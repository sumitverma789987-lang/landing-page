    ...
    return (
        <div className='w-full max-w-[1440px] mx-auto flex items-center justify-center flex-col mt-12 sm:mt-16 md:mt-20 lg:mt-24 lg:px-35 px-5'>
            ...
            <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between w-full mb-8 sm:mb-10 md:mb-12 gap-4 sm:gap-4'>
                <h2 className='text-[48px] font-bold'>From The Blog</h2>
                <button className='bg-[var(--teal)] text-white text-[16px] font-medium px-4 sm:px-6 md:px-8 py-3 md:py-4 cursor-pointer hover:bg-[#00b39a] transition-colors whitespace-nowrap'>
                    View All Blogs
                </button>
            </div>
                ...
                {BlogCards.map((item, index) => (
                    <div key={index} className='flex flex-col  overflow-hidden hover:shadow-lg transition-shadow'>
                        ...
                        <div className='pt-3 sm:pt-4 md:pt-5 px-3 sm:px-4 md:px-5 pb-3 sm:pb-4 md:pb-5 flex flex-col gap-2 sm:gap-3 flex-1'>
                            <p className='text-[14px] text-[var(--gray)]'>{item.date}</p>
                            <h3 className='text-[16px] sm:text-lg md:text-xl lg:text-2xl font-bold text-black line-clamp-2'>{item.title}</h3>
                            <p className='text-[14px] text-[var(--gray)] line-clamp-3'>{item.description}</p>
                            <button className='flex items-center gap-2 text-[var(--teal)] text-[14px] md:text-[16px] font-medium mt-1 sm:mt-2 cursor-pointer w-fit hover:text-[#00b39a] transition-colors'>
                                Read More
