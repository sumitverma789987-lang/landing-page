...
const Featured = () => {
    ...
    const [current, setCurrent] = useState(FeatureCards.length)
        ...
        <div className='flex flex-col items-center justify-center w-full max-w-[1440px] mx-auto mt-25 lg:mt-[130px] lg:px-35 px-5'>
            <div className='flex flex-row items-center justify-between w-full gap-4 sm:gap-0 mb-[50px] sm:mb-10'>
                <div className='flex flex-col items-start justify-center'>
                    <h4 className='font-[var(--font-semibold)] [font-size:var(--text-24)] md:[font-size:var(--text-48)] lg:[font-size:var(--text-48)]'>Featured Products</h4>
            ...
            <div className='overflow-hidden w-full'>
                ...
                >
                    {allFeatureCards.map((item, index) => (
                        ...
                        >
                            <div className='absolute top-2 sm:top-3 md:top-4 left-2 sm:left-3 md:left-4 bg-white text-black [font-size:var(--text-14)] md:[font-size:var(--text-16)] lg:[font-size:var(--text-16)] font-[var(--font-semibold)] px-3 sm:px-4 py-1.5 sm:py-2 rounded-full shadow-sm z-10'>
                                {item.price}
                            </div>
                            ...
                            <div className='relative flex items-center justify-center flex-1 min-h-[180px] sm:min-h-[240px] md:min-h-[441px] xl:min-w-[364px] p-4 sm:p-6 md:p-10'>
                                <img
                                    src={item.image}
                                    alt={item.title}
                                ...
                                <div className='absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center'>
                                    <button
                                        ...
                                        className='bg-[var(--teal)] cursor-pointer text-white [font-size:var(--text-14)] md:[font-size:var(--text-16)] lg:[font-size:var(--text-16)] font-[var(--font-semibold)] px-4 sm:px-6 py-2 sm:py-3 hover:bg-[#00b39a] transition-colors'
                                    >
                                        Add to Cart
                                    </button>
                                </div>
                            ...
                            <h3 className='text-center [font-size:var(--text-18)] md:[font-size:var(--text-20)] lg:[font-size:var(--text-24)] p-3 sm:p-4 md:p-5 font-[var(--font-normal)] bg-white w-full text-black pb-2'>
                                {item.title}
                            </h3>
                        </div>
                    ))}
