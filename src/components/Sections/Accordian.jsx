...
const Accordian = () => {
    ...
    return (
        <div className='flex flex-col items-center justify-center mt-[140px] lg:px-35 px-5'>
            <h1 className='text-[var(--text-24)] md:text-[var(--text-48)] lg:text-[var(--text-48)] font-[var(--font-semibold)] text-center'>Frequently Asked Questions</h1>
            <p className='text-[var(--text-14)] md:text-[var(--text-16)] lg:text-[var(--text-16)] font-[var(--font-normal)] text-[var(--gray)] mt-2 sm:mt-3 md:mt-4 text-center max-w-2xl'>Real Questions. Expert Answers. Total Confidence.</p>
                ...
                {accordion.map((item, index) => (
                    ...
                    >
                        <div className='flex flex-col'>
                            ...
                            >
                                <h4 className='font-[var(--font-medium)] text-[var(--text-16)] md:text-[var(--text-18)] lg:text-[var(--text-20)] pr-3 flex-1 break-words'>{item.title}</h4>
                                <span className={`ml-2 sm:ml-3 flex-shrink-0 transition-transform duration-200 ${activeIndex === index ? 'rotate-180' : ''}`}>
                                    <Arrow />
                                </span>
                            </button>
                            {activeIndex === index && (
                                <p className='font-[var(--font-normal)] text-[var(--gray)] text-[var(--text-14)] md:text-[var(--text-16)] lg:text-[var(--text-16)] mt-3 sm:mt-4 md:mt-5 leading-relaxed'>{item.description}</p>
                            )}
                        </div>
                    </div>
                ))}
